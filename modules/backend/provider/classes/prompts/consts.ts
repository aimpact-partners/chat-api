...
const POST =
	'Si tienes sugerencias, dudas, comentarios, o cualquier otro contenido de texto ' +
...
const PROMPT_BULLETS =
	`La finalidad del contenido solicitado es la de ser utilizado para un análisis posterior` +
...
export const prompts = {
	class: {
		synthesis: (curriculumObjective: string, topics: string[]) =>
			`${CLASS_SYNTHESIS}\n\n` +
			`OBJETIVO CURRICULAR = [${curriculumObjective}]\n\n` +
			`TEMAS = [\n* ${topics.join('\n* ')}\n]`,
		assessment: (curriculumObjective: string, topics: string[]) =>
			`${CLASS_ASSESSMENT}\n\n` +
			`OBJETIVO CURRICULAR = [${curriculumObjective}]\n\n` +
			`TEMAS = [\n* ${topics.join('\n* ')}\n]`,
		relevance: (curriculumObjective: string, topics: string[]) =>
			`${CLASS_ASSESSMENT}\n\n` +
			`OBJETIVO CURRICULAR = [${curriculumObjective}]\n\n` +
			`TEMAS = [\n* ${topics.join('\n* ')}\n]`,
	},
	topic: {
		synthesis: (curriculumObjective: string, topic: string) =>
			`${TOPIC_SYNTHESIS}\n\n` + `OBJETIVO CURRICULAR = [${curriculumObjective}]\n\n` + `TEMA = [${topic}]`,
		assessment: (curriculumObjective: string, topic: string) =>
			`${TOPIC_ASSESSMENT}\n\n` + `OBJETIVO CURRICULAR = [${curriculumObjective}]\n\n` + `TEMA = [${topic}]`,
		preRequisitesAssessment: (curriculumObjective: string, topic: string) =>
			`${TOPIC_PRE_REQUISITES}\n\n` + `OBJETIVO CURRICULAR = [${curriculumObjective}]\n\n` + `TEMA = [${topic}]`,
	},
};
